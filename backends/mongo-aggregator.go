...
func (self *MongoBackend) Sum(collection string, field string, f ...*filter.Filter) (float64, error) {
	return self.aggregateFloat(collection, filter.Sum, field, f)
}

func (self *MongoBackend) Count(name string, flt ...*filter.Filter) (uint64, error) {
...
func (self *MongoBackend) Minimum(collection string, field string, f ...*filter.Filter) (float64, error) {
	return self.aggregateFloat(collection, filter.Minimum, field, f)
}

func (self *MongoBackend) Maximum(collection string, field string, f ...*filter.Filter) (float64, error) {
	return self.aggregateFloat(collection, filter.Maximum, field, f)
}

func (self *MongoBackend) Average(collection string, field string, f ...*filter.Filter) (float64, error) {
	return self.aggregateFloat(collection, filter.Average, field, f)
}

func (self *MongoBackend) GroupBy(collection string, groupBy []string, aggregates []filter.Aggregate, flt ...*filter.Filter) (*dal.RecordSet, error) {
...
func (self *MongoBackend) aggregateFloat(name string, aggregation filter.Aggregation, field string, flt []*filter.Filter) (float64, error) {
...
func (self *MongoBackend) aggregate(name string, groupBy []string, aggregates []filter.Aggregate, flt []*filter.Filter, single bool) (interface{}, error) {
	if collection, err := self.GetCollection(name); err == nil {
		var f *filter.Filter
